...
export default async function PropertyDetailPage({ params }: PageProps) {
  ...
  const activeLease = (leases ?? []).find((l) => l.status === 'active') as
    | (Lease & { tenants: { name: string; email: string } })
    | undefined;
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      ...
      <div className="border rounded-lg p-4 flex flex-col gap-1">
        ...
        <p className="font-medium">${Number(property.monthly_rent).toLocaleString()}/mo</p>
      </div>
      ...
      {activeLease && (
        <div className="border rounded-lg p-4 flex flex-col gap-2">
          ...
          <p className="text-sm">Tenant: {activeLease.tenants?.name} — {activeLease.tenants?.email}</p>
          <p className="text-sm">
          ...
          {leases.map((lease) => (
            <div key={lease.id} className="border rounded-lg p-3 flex flex-col gap-1 text-sm">
              <div className="flex justify-between items-center">
                <span className="font-medium">{(lease as Lease & { tenants: { name: string } }).tenants?.name}</span>
