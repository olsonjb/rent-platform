...
export function buildScreeningPrompt(ctx: ScreeningPromptContext): string {
  return `You are a tenant screening AI for residential rentals. Evaluate this rental application and make an approval/denial recommendation.
...
- Social Media Links: ${ctx.socialMediaLinks.length > 0 ? ctx.socialMediaLinks.join(', ') : 'None provided'}
